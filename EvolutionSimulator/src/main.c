...
void main_preinit() {
	srand48 (time(NULL));windowTitle = "Evolution Simulator";
	frameLimit = 60;
	fitnessName = "Distance";
	minBar = -10;
	maxBar = 100;
	barLen = maxBar - minBar;
	histBarsPerMeter = 5;
	fitnessUnit = "m";
	gravity = .005;
	airFriction = .95;
	...
	friction = 4;
	haveGround = 1;
	hazelStairs = -1.;bigMutationChance = .06;
	energyDirection = 1;
	lineY1 = -.08;
	lineY2 = .35;
	tps = 60;
...
void main_init() {
	...
	guistate_register_render(0, evol_render);
	guistate_register_load(0, evol_init);
	guistate_register_button(0, evol_button);
	guistate_register_render(1, mkcr_render);
	guistate_register_load(1, mkcr_init);
	guistate_register_button(1, mkcr_button);
	guistate_register_render(2, vcrs_render);
	guistate_register_load(2, vcrs_init);
	guistate_register_button(2, vcrs_button);
	guistate_register_tick(2, vcrs_stick);
	guistate_register_render(3, isim_render);
	guistate_register_load(3, isim_init);
	guistate_register_button(3, isim_button);
	guistate_register_tick(3, isim_tick);
